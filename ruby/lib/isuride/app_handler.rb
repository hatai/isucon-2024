  ...
  class AppHandler < BaseHandler
    ...
    get '/rides' do
      items = db_transaction do |tx|
        rides = tx.xquery('SELECT id, chair_id, pickup_latitude, pickup_longitude, destination_latitude, destination_longitude FROM rides WHERE user_id = ? ORDER BY created_at DESC', @current_user.id)
