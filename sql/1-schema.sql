...
(
  ...
  created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '登録日時',
  PRIMARY KEY (id),
    INDEX idx_chair_locations_chair_id_created_at (chair_id, created_at DESC)
)
...
(
  ...
  PRIMARY KEY (id),
  INDEX idx_ride_statuses_ride_id_creat (ride_id, created_at DESC),
  INDEX idx_ride_appsent_created (ride_id, app_sent_at, created_at)
)
